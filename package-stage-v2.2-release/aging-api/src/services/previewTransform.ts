 ...
 */
function stableIdBase(tx: any): string {
...
function deterministicId(tx: any, duplicateBaseIds: Set<string>): string {
  const baseId = stableIdBase(tx);
  if (!duplicateBaseIds.has(baseId)) return baseId;
  return `${baseId}:r${Number(tx.sourceRow ?? 0)}`;
}

function findDuplicateBaseIds(rawTransactions: any[]) {
 ...
 */
function toPreviewTransaction(tx: any, duplicateBaseIds: Set<string>) {
...
function buildTxIndex(previewTransactions: ReturnType<typeof buildPreviewTransactions>) {
  ...
  const byTypeRow = new Map<string, string | null>();  // sourceType|sourceRow → tx.id, null if ambiguous
