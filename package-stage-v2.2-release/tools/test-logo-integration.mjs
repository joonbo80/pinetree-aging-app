...
const headerBytes = readBytes(headerPath);
const icoBytes = readBytes(faviconIcoPath);
...
check('public favicon.ico exists', fileExists(faviconIcoPath, 1000));
...
check('Header.tsx has no UTF-8 BOM', !(headerBytes[0] === 0xef && headerBytes[1] === 0xbb && headerBytes[2] === 0xbf));
check('favicon.ico has ICONDIR reserved=0', icoBytes.readUInt16LE(0) === 0);
check('favicon.ico has ICO type=1', icoBytes.readUInt16LE(2) === 1);
check('favicon.ico has sane icon count', icoBytes.readUInt16LE(4) >= 1 && icoBytes.readUInt16LE(4) <= 16);
check('favicon.ico contains 16px entry', icoBytes.readUInt8(6) === 16 || icoBytes.readUInt8(22) === 16 || icoBytes.readUInt8(38) === 16);
check('favicon.ico contains 32px entry', icoBytes.readUInt8(6) === 32 || icoBytes.readUInt8(22) === 32 || icoBytes.readUInt8(38) === 32);
check('favicon.ico contains 48px entry', icoBytes.readUInt8(6) === 48 || icoBytes.readUInt8(22) === 48 || icoBytes.readUInt8(38) === 48);
