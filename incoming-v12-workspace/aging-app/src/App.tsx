...
import { UploadNotSupportedModal } from './components/UploadNotSupportedModal';
...
export default function App() {
  ...
  // Upload-not-supported modal (P1 fix)
  ...
  const handleFiles = (files: File[]) => {
    // P1 SAFETY: Live .xls/.xlsx parsing is NOT supported in this build.
    // Silently routing the drop to baseline would create a dangerous illusion
    // that the user's accounting files were analyzed. Show an explicit modal
    // instead. The user must opt in to "Load demo baseline" separately.
  ...
  return (
    <div className="app-shell">
      ...
      <UploadNotSupportedModal
        lang={lang}
        open={showUploadDisabled}
        files={pendingFiles}
        onClose={() => {
          setShowUploadDisabled(false);
          setPendingFiles([]);
        }}
        onLoadDemo={() => {
          setPendingFiles([]);
