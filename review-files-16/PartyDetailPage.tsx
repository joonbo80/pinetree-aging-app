...
type TxDirectionFilter = 'ALL' | 'receivable' | 'payable' | 'settled';
type TxSortMode =
...
function TransactionsTab({ detail }: { detail: PartyDetail }) {
  ...
  const [direction, setDirection] = useState<TxDirectionFilter>('ALL');
  ...
  const rows = useMemo(() => {
    ...
    if (direction !== 'ALL') r = r.filter(t => t.direction === direction);
...
function sortTransactions(
  ...
  switch (mode) {
    ...
    case 'date-desc':
      ...
      break;
    case 'date-asc':
      out.sort((a, b) => (a.invoiceDate ?? '').localeCompare(b.invoiceDate ?? ''));
      break;
    case 'source-row-asc':
      out.sort((a, b) => {
