...
`npm run dev` and `node dist/server.js` are the supported launch paths. Keep the
UI on an HTTP server as well; direct `file://` preview is not supported by the
React BrowserRouter dashboard route.
...
Deployment note: live upload parsing is a mixed Node + Python runtime. Package
both runtimes together, or set `AGING_PYTHON` to a Python executable that can
import the Phase 1 workbook dependencies.
