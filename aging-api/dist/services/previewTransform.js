 ...
 *
 * Same input row always produces the same id. Survives re-parses and
 * source file renames. For normal rows it also survives row reordering
 * because sourceRow is not part of the primary id.
 *
 * Format:
 *   default:   `<sourceType>:<sourceIdentityKey>:<sourceContentHash>`
 *   collision: `<sourceType>:<sourceIdentityKey>:<sourceContentHash>:r<sourceRow>`
 *
 ...
 *  - sourceRow:         appended only when a base id collides inside the
 *                       current payload. This keeps ordinary ids stable
 *                       when a re-export inserts rows above them, while
 *                       still giving true duplicate rows distinct FK ids.
 *
...
function stableIdBase(tx) {
    ...
    return `${sourceType}:${sourceIdentityKey}:${hashShort}`;
}
function deterministicId(tx, duplicateBaseIds) {
    const baseId = stableIdBase(tx);
    if (!duplicateBaseIds.has(baseId))
        return baseId;
    return `${baseId}:r${Number(tx.sourceRow ?? 0)}`;
}
function findDuplicateBaseIds(rawTransactions) {
    const baseCounts = new Map();
    for (const tx of rawTransactions) {
        const baseId = stableIdBase(tx);
        baseCounts.set(baseId, (baseCounts.get(baseId) ?? 0) + 1);
    }
    return new Set([...baseCounts.entries()].filter(([, count]) => count > 1).map(([baseId]) => baseId));
}
/**
...
function bucketLabel(b) {
    ...
    if (b === '0-30' || b === '31-60' || b === '61-90' || b === '90+')
        return b;
    return '0-30';
}
...
function toDirection(value) {
    if (value === 'payable' || value === 'settled')
        return value;
    return 'receivable';
}
function toSourceType(value) {
    if (value === 'CRDR' || value === 'AP')
        return value;
    return 'INVOICE';
}
/**
 ...
 */
function toPreviewTransaction(tx, duplicateBaseIds) {
    const partyDisplay = String(tx.partyName ?? tx.partyNameRaw ?? '');
    return {
        id: deterministicId(tx, duplicateBaseIds),
        rawId: String(tx.id ?? ''),
...
function buildPreviewTransactions(rawTransactions) {
    const duplicateBaseIds = findDuplicateBaseIds(rawTransactions);
    return rawTransactions.map(tx => toPreviewTransaction(tx, duplicateBaseIds));
}
/**
 ...
 */
function indexById(previewTransactions) {
    const map = new Map();
 ...
 */
function buildReviewItems(rawValidationReport, previewTransactions, duplicateGroups, statementLinks, strictNotInErpRowSet) {
    const byId = indexById(previewTransactions);
    const items = [];
    let nextId = 1;
    const addFromTx = (tx, category, severity, reason, reasonCode, extra = {}) => {
        items.push({
    ...
    for (const w of rawValidationReport?.warnings ?? []) {
        ...
        if (Array.isArray(w.affectedTransactions)) {
            for (const txId of w.affectedTransactions) {
                ...
                if (tx)
                    addFromTx(tx, 'WARNINGS', 'warning', message, reasonCode);
            }
        }
            ...
            for (const c of w.affectedCompanies) {
                items.push({
                    ...
                    partyName: String(c.partyName ?? c.party ?? ''),
                    currency: null,
                    amount: null,
                    trace: null,
                    details: {
    ...
    for (const tx of previewTransactions) {
        if (tx.agingBucket === '90+' && tx.direction !== 'settled') {
            addFromTx(tx, 'AGING_90_PLUS', 'warning', `Aging ${tx.agingDays} days (${tx.agingBucket})`, 'AGING_90_PLUS', { agingDays: tx.agingDays });
        }
    }
    ...
    for (const group of duplicateGroups) {
        for (const txId of group.transactionIds) {
            ...
            if (tx) {
                addFromTx(tx, 'DUPLICATES', 'warning', `Duplicate of ${group.identityKey} (group of ${group.count})`, 'W1_DUPLICATE', {
    ...
    for (const link of statementLinks) {
        ...
        if (!strictNotInErpRowSet.has(`${link.sourceFile}|${link.sourceRow}`))
            continue;
        items.push({
            id: `R-${String(nextId++).padStart(5, '0')}`,
            ...
            details: {
                ...
                referenceStatus: link.referenceStatus,
                differenceType: link.differenceType,
            },
        });
    }
    // --- UNKNOWN_DEPARTMENT ---
    for (const tx of previewTransactions) {
        if (tx.department === null || tx.department === 'UNKNOWN') {
            addFromTx(tx, 'UNKNOWN_DEPARTMENT', 'info', 'Department could not be classified', 'UNKNOWN_DEPARTMENT', { rawDepartment: null });
        }
    }
    return items;
}
 ...
 */
function buildDuplicateGroupDetails(rawTransactions) {
    const groups = new Map();
    const duplicateBaseIds = findDuplicateBaseIds(rawTransactions);
    for (const tx of rawTransactions) {
        if (!Array.isArray(tx.anomalyRefs) || !tx.anomalyRefs.includes('W1'))
        ...
        group.transactionIds.push(deterministicId(tx, duplicateBaseIds));
    ...
    for (const g of groups.values()) {
        if (g.transactionIds.length < 2)
            continue;
        if (g.zeroFlags.every(z => z))
            continue; // all-zero-balance group: skip
 ...
 * AGENT `crdrSourceRow`.
 */
function buildTxIndex(previewTransactions) {
    const byCurRef = new Map(); // sourceType|currency|ourRefNo → tx.id
    const byCurInv = new Map(); // sourceType|currency|invoiceNo → tx.id
    const byCurCrdr = new Map(); // CRDR|currency|crdrNo → tx.id
    const byTypeRow = new Map(); // sourceType|sourceRow → tx.id, null if ambiguous
    for (const tx of previewTransactions) {
        const cur = tx.currency;
        const st = tx.sourceType;
        if (tx.ourRefNo)
            byCurRef.set(`${st}|${cur}|${tx.ourRefNo}`, tx.id);
        if (tx.invoiceNo)
            byCurInv.set(`${st}|${cur}|${tx.invoiceNo}`, tx.id);
        if (tx.crdrNo && st === 'CRDR')
            byCurCrdr.set(`CRDR|${cur}|${tx.crdrNo}`, tx.id);
        const rowKey = `${st}|${tx.trace.sourceRow}`;
        if (byTypeRow.has(rowKey) && byTypeRow.get(rowKey) !== tx.id) {
            byTypeRow.set(rowKey, null);
        }
        else {
            byTypeRow.set(rowKey, tx.id);
        }
    }
 ...
 *   5. null
 */
function localMatchToLink(stmt, m, idx) {
    ...
    return {
        ...
        partyName: String(stmt.partyName ?? ''),
        ...
        referenceStatus: m.referenceStatus ?? null,
        differenceType: m.differenceType ?? null,
    };
}
/**
 ...
 *   3. null
 */
function agentMatchToLink(stmt, m, idx) {
 ...
 */
function localMatchType(m) {
    const refStatus = String(m.referenceStatus ?? '');
    const diffType = String(m.differenceType ?? '');
    if (refStatus === 'not_in_uploaded_erp_extract')
        return 'NOT_IN_ERP_EXTRACT';
    if (diffType === 'OUTSIDE_UPLOADED_ERP_DATE_RANGE')
        return 'OUTSIDE_DATE_RANGE';
    if (diffType === 'SAME_REF_DIFFERENT_CURRENCY')
        return 'CURRENCY_MISMATCH';
    if (diffType === 'NO_REFERENCE_NUMBER')
        return 'NO_REFERENCE';
    ...
    if (refStatus === 'found' && diffType && diffType !== 'None')
 ...
 *   differenceType
 */
function agentMatchType(m) {
    if (m.identityMatched === false)
        return 'IDENTITY_MISMATCH';
    if (m.currencyMatches === false)
        return 'CURRENCY_MISMATCH';
    if (m.currentBalanceStatus === 'settled_in_erp')
        return 'SETTLED_AFTER_STATEMENT';
    if (m.currentBalanceStatus === 'changed_in_erp')
        return 'CHANGED_AFTER_STATEMENT';
    if (m.differenceType === 'AS_OF_DATE_MISMATCH')
        return 'AS_OF_DATE_MISMATCH';
    if (m.matched === false)
        return 'NO_REFERENCE';
    if (Math.abs(Number(m.balanceDifference ?? 0)) < 0.005)
        return 'EXACT_SIGNED';
    return 'BALANCE_DIFFERENCE';
}
/**
 ...
 */
function buildStatementLinks(rawStatementMatchReport, previewTransactions) {
...
function buildDetails(raw) {
    ...
    const strictNotInErpRowSet = new Set();
    ...
    const reviewItems = buildReviewItems(raw.validationReport ?? {}, transactions, duplicateGroups, statementLinks, strictNotInErpRowSet);
