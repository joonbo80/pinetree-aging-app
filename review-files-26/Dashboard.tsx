...
interface DashboardProps {
  ...
  /**
   * Step 12: navigate to /party/:partyKey when a Top Party row is clicked.
   * Following the existing onOpenReview / onOpenReviewCategory callback
   * pattern rather than introducing react-router-dom into Dashboard.
   */
...
export function Dashboard({
  ...
  // Step 12: build a partyName -> partyKey lookup as fallback for
  // legacy payloads that don't have TopPartySummary.partyKey populated.
  // Uses details.transactions where each row has both fields. v2.2+
  // payloads should populate partyKey directly so this map is unused.
  ...
  const resolvePartyKey = (row: { partyKey?: string; partyName: string }): string | null =>
    row.partyKey ?? nameToKey.get(row.partyName) ?? null;
    ...
    <main className="app-content dashboard-page">
      ...
      <section className="dashboard-grid two-col">
        ...
        <div className="dash-panel">
          ...
          <TopPartyTable title="Top Receivable" rows={receivableParties.slice(0, 6)} onOpenParty={onOpenParty} resolvePartyKey={resolvePartyKey} />
          <TopPartyTable title="Top Payable" rows={payableParties.slice(0, 6)} onOpenParty={onOpenParty} resolvePartyKey={resolvePartyKey} />
        </div>
      </section>
    </main>
  );
...
function TopPartyTable({
  ...
  onOpenParty?: (partyKey: string) => void;
  resolvePartyKey: (row: { partyKey?: string; partyName: string }) => string | null;
}) {
  return (
    <div className="top-party-section">
      ...
      <table className="data compact">
        ...
        <tbody>
          {rows.map(row => {
            const key = resolvePartyKey(row);
            const canLink = onOpenParty && key !== null;
            return (
              <tr
                key={`${row.partyName}-${row.currency}-${row.direction}`}
                className={canLink ? 'top-party-row top-party-row-link' : 'top-party-row'}
              >
                <td>
                  {canLink ? (
                    <button
                      type="button"
                      className="top-party-link"
                      onClick={() => onOpenParty!(key!)}
                      aria-label={`Open party detail for ${row.partyName}`}
                    >
                      <b>{row.partyName}</b>
                      <br />
