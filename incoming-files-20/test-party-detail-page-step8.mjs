...
check(`trace shows source row`,                  source.includes('String(item.trace.sourceRow)'));
check(`trace shows party name (v2.1.1 invariant)`, source.includes('"Party Name"'));
check(`trace shows reason code separately from human reason`,
      source.includes('"Reason Code"') && source.includes('item.reasonCode'));
check(`trace shows severity`,                    source.includes('"Severity"'));
