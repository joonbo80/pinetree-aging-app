# v2.3 C2 Round 1 Fix Package -- Selector + Data Invariants
...
If distributed as a zip, the recommended filename is
`v2.3-c2-round1-fix-package.zip`, but the canonical unit is the
folder itself.
...
```
BROKEN state (selector source file absent): PASS: 14    FAIL: 10
FIXED  state (selector source file present): PASS: 24   FAIL:  0
```

The 14 PASSes on broken state are runtime invariants that grep
the baseline fixture directly. They check the baseline matches
Section 1.4 numbers, regardless of whether the selector exists.
This is intentional: those invariants pin baseline reality, not
selector implementation. If the baseline ever drifts, those
invariants catch it even with a working selector.

The 10 FAILs on broken state are source-level invariants that
require the selector .ts file to exist and have the correct
structure. They pin the selector implementation against the spec.

Together, the two layers protect against:

- Selector drift (source-level invariants 1-7 FAIL if selector
  removed or mutated incorrectly).
- Baseline drift (runtime invariants 8-11 FAIL if the v1.3.0
  fixture is regenerated with different data).
...
- [ ] `node .\tools\test-aging-report-view.mjs` returns PASS: 24 / FAIL: 0.
