...
5. Restart dev server. SAFE method first:

   In each existing PowerShell window running Vite or the API server,
   press Ctrl+C to stop that specific process. Wait for the prompt
   to return. This is the recommended approach because it only
   affects the dev servers you started in those windows.
...
```
   ...
   If Vite still falls back to 5174, port 5173 is held by another
   process. Identify it before killing anything:

```powershell
netstat -ano | findstr :5173
```

   The last column is the PID. Check who owns it:

```powershell
Get-Process -Id <PID>
```

   Only after confirming the PID is a stale dev server (not VS Code,
   not Claude Code, not another project) should you stop it:
...
```
   ...
   unless you have confirmed no other Node-based tools are running.
