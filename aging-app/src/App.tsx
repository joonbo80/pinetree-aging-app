...
async function loadBundledBaseline(): Promise<ParsingPreviewResult> {
  const base = import.meta.env.BASE_URL.endsWith('/')
    ? import.meta.env.BASE_URL
    : `${import.meta.env.BASE_URL}/`;
  const response = await fetch(`${base}baselines/phase1-v1.3.0.json`);
  if (!response.ok) {
    throw new Error(`Bundled baseline unavailable (${response.status})`);
  }
  return response.json() as Promise<ParsingPreviewResult>;
}
...
export default function App() {
  ...
  const handleLoadBaseline = async () => {
    ...
    try {
      ...
      // into the main JavaScript bundle.
      const bundledBaseline = await loadBundledBaseline();
      setResult(bundledBaseline);
