...
import type { ParsingPreviewResult, DirectionTotal } from '../../parsing-engine/types';
...
export function ReconciliationTab({ lang, result }: ReconciliationTabProps) {
  ...
  const directionLabels: Record<DirectionTotal['direction'], string> = {
          ...
          list.map(d => (
            <div key={`${currency}-${d.direction}`} className="direction-cell">
              <div className="direction-cell-head">
                <span>{directionLabels[d.direction]}</span>
                <b>{currency}</b>
              </div>
