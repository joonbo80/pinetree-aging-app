...
function triggerDownload(csv: string, filename: string): void {
  ...
  //   - Anchor must be DOM-attached for reliable click() in some
  //     browsers (Firefox in particular).
  //   - revokeObjectURL is deferred via setTimeout(..., 0) so the
  //     current event loop completes the click + download dispatch
