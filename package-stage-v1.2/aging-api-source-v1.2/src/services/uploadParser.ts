...
import type { UploadManifest, UploadedWorkbookFile } from '../types.js';
...
export async function parseWorkbookMultipart(req: Request): Promise<ParsedUpload> {
  ...
  const files: UploadedWorkbookFile[] = [];
  const pending: Promise<void>[] = [];
  let totalBytes = 0;
  let rejected: UploadError | null = null;

  const cleanup = async () => {
    await rm(tempDir, { recursive: true, force: true });
  };

  const rejectOnce = (err: UploadError) => {
  ...
  await new Promise<void>((resolvePromise, rejectPromise) => {
    ...
    bb.on('file', (_fieldName, file, info) => {
      ...
      pending.push(new Promise<void>((resolveFile, rejectFile) => {
  ...
  try {
    ...
    if (files.length === 0) throw new UploadError('NO_FILES', 'No workbook files were uploaded', 400);
    return {
      tempDir,
      manifest: {
        importBatchId,
