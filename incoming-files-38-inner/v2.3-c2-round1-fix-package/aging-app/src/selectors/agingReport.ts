...
/**
 * Group rows by (partyKey, currency). A party with both USD and CAD open
 * balances appears as two rollup rows (currency LOCKED rule).
 */
function buildTabData(
  rows: TransactionRow[],
  includeBuckets: boolean,
): AgingTabData {
  // Group by partyKey + currency
  const groups = new Map<string, TransactionRow[]>();
  for (const r of rows) {
    const key = `${r.partyKey}__${r.currency}`;
  ...
  for (const groupRows of groups.values()) {
    ...
    if (first.currency === 'USD') totalUSD += openAmount;
    else totalCAD += openAmount;
  }
