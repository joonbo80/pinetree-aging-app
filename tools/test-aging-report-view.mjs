...
section('inv-aging-data-build-hygiene: TypeScript strict check guards');
// rev2 -> rev3: buildTransactionRow had an unused asOfDate parameter that
// broke `npm run build` under tsconfig's noUnusedParameters: true. Pin
// the signature so the regression cannot return silently.
//
// The expected signature is exactly 3 parameters:
//   tx: PreviewTransaction
//   stmtByTxId: Map<string, StatementStatus>
//   duplicateTxIds: Set<string>
// Counting commas naively breaks on generic types (Map<K, V>); instead
// we assert the exact closing signature.
check(
  'inv-aging-data-build-hygiene-1: buildTransactionRow signature ends with duplicateTxIds (no extra param)',
  /function\s+buildTransactionRow\s*\(\s*tx\s*:\s*PreviewTransaction,\s*stmtByTxId\s*:\s*Map<[^>]+>,\s*duplicateTxIds\s*:\s*Set<[^>]+>,?\s*\)\s*:/.test(
    source,
  ),
);
check(
  'inv-aging-data-build-hygiene-2: buildTransactionRow does NOT declare asOfDate param',
  !/function\s+buildTransactionRow[\s\S]{0,300}?asOfDate\s*:/.test(source),
);
