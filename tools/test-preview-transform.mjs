...
console.log(`\n=== id format ===`);
const idPattern = /^(INVOICE|CRDR|AP):[^:]+:[a-f0-9]+(?::r\d+)?$/;
...
// Check 7.6: id is independent of sourceRow for non-collision rows.
//
// Duplicate/collision ids intentionally append :r<sourceRow>. Ordinary ids
// should remain stable if a re-export inserts rows above them.
console.log(`\n=== id independence from sourceRow for non-collisions ===`);
const normalIds = [...ids].filter(id => !/:r\d+$/.test(id));
const rowShiftCopy = JSON.parse(JSON.stringify(raw));
for (const tx of rowShiftCopy.transactions) {
  tx.sourceRow = Number(tx.sourceRow ?? 0) + 100;
}
const rowShiftResult = toParsingPreviewResult(rowShiftCopy);
const rowShiftIds = new Set(rowShiftResult.details.transactions.map(t => t.id));
let rowStableCount = 0;
for (const id of normalIds) if (rowShiftIds.has(id)) rowStableCount++;
console.log(`Non-collision ids preserved after row shift: ${rowStableCount}/${normalIds.length}`);
if (rowStableCount !== normalIds.length) {
  console.error(`  FAIL: non-collision ids changed when sourceRow changed`);
  allPass = false;
} else {
  console.log('  PASS');
}
