 ...
 *
 * Same input row always produces the same id. Survives re-parses, enables
 * v3 persistence to reference the row across import sessions.
 *
 * Format: `<sourceType>:<sourceFile>#<sourceRow>:<contentHashShort>`
 *
 * Why each part is included:
 *  - sourceType: distinguishes the same row number across files
 *  - sourceFile: distinguishes same row number across files of same type
 *  - sourceRow:  distinguishes rows within the same file
 *  - contentHash short prefix: distinguishes "row 47 today" from "row 47
 *    after the user re-exported the file with one row inserted above"
 *
 * If sourceContentHash is missing on the raw row, we fall back to a stable
 * hash of (sourceType, sourceFile, sourceRow, sourceIdentityKey).
 */
function deterministicId(tx: any): string {
  const sourceType = String(tx.sourceType ?? 'UNKNOWN');
  const sourceFile = String(tx.sourceFile ?? '');
  const sourceRow = Number(tx.sourceRow ?? 0);
  let hashShort = String(tx.sourceContentHash ?? '').slice(0, 12);
  if (!hashShort) {
    // Fallback: deterministic hash of the identifying fields. We use a
    // tiny FNV-1a so we don't pull in node:crypto at the top of a hot
    // projection path; collisions across the small input space are
    // effectively zero.
    const probe = `${sourceType}|${sourceFile}|${sourceRow}|${tx.sourceIdentityKey ?? ''}`;
    let h = 0x811c9dc5;
  ...
  return `${sourceType}:${sourceFile}#${sourceRow}:${hashShort}`;
}
 ...
 *   1. candidateRows[0] — Phase 1 already picked the best candidate
 *      with sourceType + sourceRow. Use it directly.
 *   2. INVOICE+currency+ourRefNo
 *   3. INVOICE+currency+invoiceNo
 *   4. null
 */
function localMatchToLink(stmt: any, m: any, idx: TxIndex) {
  const currency = toCurrency(m.currency ?? stmt.currency ?? 'USD');

  // Strategy 1: candidateRows precise hit
  ...
  // Strategy 2/3: scoped reference lookup, INVOICE primary
