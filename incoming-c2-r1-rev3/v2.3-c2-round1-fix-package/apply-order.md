...
```
BROKEN state (selector absent):   PASS:  7    FAIL: 14
FIXED  state (selector present):  PASS: 38    FAIL:  0
```

rev3 added 2 build-hygiene invariants (signature pin) to catch the
unused-parameter regression that broke `npm run build` under
tsconfig's `noUnusedParameters: true`. The build-hygiene invariants
fail in a regression state where the selector adds an unused
parameter, so the build break is caught BEFORE `npm run build`
fires.
