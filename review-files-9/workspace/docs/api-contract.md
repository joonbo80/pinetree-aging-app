...
**v1.2:** accepts multipart `.xls` / `.xlsx` uploads and returns a
`ParsingPreviewResult`.

Required request header in v1.2 development builds:
...
```json
{ "error": "Upload authorization required", "code": "UPLOAD_UNAUTHORIZED" }
```

If the server has no `AGING_UPLOAD_TOKEN` configured:

```json
{ "error": "Upload endpoint is not configured", "code": "UPLOAD_AUTH_NOT_CONFIGURED" }
```

This token is a temporary development gate. Production auth must be
Microsoft Entra / Teams SSO; do not treat the token as a long-term auth
model.
...
- Request files are saved under an API-managed temp directory with generated
  filenames; client filenames are metadata only.
- Temp directories are removed in `finally` after success or failure.
- The request handler never imports the Phase 1 parser directly. It spawns
  `aging-api/scripts/phase1-worker.mjs` with a wall-clock timeout and
  `--max-old-space-size=512`, then converts the raw result to
  `ParsingPreviewResult`.
- Runtime config:
  - `AGING_UPLOAD_TOKEN`: required by `/api/parse-upload`
  - `AGING_PYTHON`: Python executable used by `tools/extract_workbook.py`
  - `AGING_PHASE1_ROOT`: optional path to the repo root containing
    `parsing-engine/` and `tools/`
