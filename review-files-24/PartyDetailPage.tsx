...
import { exportPartyTabCsv } from './csvExport';
...
function TransactionsTab({
  ...
  // (focusedTx lookup not needed — filter logic uses focusedTxId directly)
  ...
  }, [detail.transactions, currency, direction, focusedTxId, query, sortMode]);
  ...
  return (
    <div className="party-tab-content">
      ...
      <div className="party-tab-footer">
        ...
        <button
          type="button"
          className="btn party-tab-export-btn"
          onClick={() => exportPartyTabCsv(detail, 'transactions')}
...
function TransactionTracePanel({
  ...
  // Build domain references list (only present ones — suppress noise)
  const refFields: TraceEntry[] = [];
  if (tx.ourRefNo)         refFields.push({ label: 'ourRefNo',        value: tx.ourRefNo,        mono: true });
  if (tx.invoiceNo)        refFields.push({ label: 'invoiceNo',       value: tx.invoiceNo,       mono: true });
  if (tx.crdrNo)           refFields.push({ label: 'crdrNo',          value: tx.crdrNo,          mono: true });
  if (tx.vendorInvoiceNo)  refFields.push({ label: 'vendorInvoiceNo', value: tx.vendorInvoiceNo, mono: true });
  if (tx.vendorName)       refFields.push({ label: 'vendorName',      value: tx.vendorName,      mono: true });
  if (tx.blNo)             refFields.push({ label: 'blNo',            value: tx.blNo,            mono: true });

  const model: TracePanelModel = {
    source: {
      file: tx.trace.sourceFile,
      sheet: tx.trace.sourceSheet || '—',
      row: tx.trace.sourceRow,
    },
    identity: {
      fields: [
        { label: 'Transaction ID', value: tx.id,        mono: true, small: true },
        { label: 'Party Name',     value: partyName },
        { label: 'Party Key',      value: tx.partyKey,  mono: true },
        { label: 'Department',     value: tx.department ?? 'UNKNOWN' },
        { label: 'Aging Basis',    value: tx.agingBasisDate, mono: true },
        { label: 'Raw Balance',    value: formatMoney(tx.rawBalance), mono: true },
        { label: 'Signed',         value: formatSignedMoney(tx.signedBalance), mono: true },
        { label: 'Zero Balance',   value: tx.isZeroBalance ? 'yes' : 'no' },
        { label: 'Flags',          value: tx.flags.length > 0 ? tx.flags.join(', ') : '—', mono: true },
      ],
    },
    details: refFields.length > 0
      ? [{ label: 'REFERENCES', fields: refFields }]
      : [],
  };

  return <TracePanel model={model} />;
}
...
// ============================================================
// Step 10 — Unified Trace Panel (Option C)
// ============================================================
//
// One parameterized component replacing the 3 per-tab trace panels
// (TransactionTracePanel / StatementTracePanel / ReviewTracePanel).
//
// Per reviewer guidance (Phase 3 Collection Action System awareness):
//
//   - This is a PURE REFACTOR for v2.2. No new UI. No data model
//     change. No spec change.
//   - Props/model shape is reorganized into named sections so that
//     Phase 3's Collection Activity / Status / NextAction can attach
//     naturally without redesigning.
//   - We do NOT introduce event/status enums in v2.2. Those belong
//     to Phase 3 micro-spec when the SQL/email/Teams domain is
//     designed end-to-end.
//
// Sections (v2.2 scope):
//   source        — origin: file / sheet / row (always present)
//   identity      — record identifiers: partyName, partyKey, tx id,
//                   statement source LOCAL/AGENT, etc.
//   linkedRecord  — cross-tab focus target (e.g. statement -> matched
//                   ERP tx). Renders a section + jump button.
//   details       — domain references and status fields: invoiceNo,
//                   matchType, severity, raw statement status, etc.
//   notes         — long-form prose, e.g. review reason text.
//
// Reserved (Phase 3 — names only, intentionally unused in v2.2):
//   activity?     — list of Collection Activities (emails sent,
//                   payment promises, status changes)
//   status?       — current Collection Status enum
//   nextAction?   — recommended/scheduled next step
//
// The component renders nothing for these reserved slots; passing
// them to TracePanel today is a noop. When Phase 3 wires them, the
// component grows new sections without breaking call sites.
...
interface TraceLinkedRecord {
  /** Section heading, e.g. "ERP MATCH" or "LINKED TRANSACTION". */
  label: string;
  /** Field grid shown above the jump button. */
  fields: TraceEntry[];
  /** The transaction id to focus when the user clicks the jump button. */
  jumpTxId: string;
  /** Button label, e.g. "View this transaction in Transactions tab →". */
  jumpLabel: string;
}

interface TraceDetailGroup {
  /** Section heading, e.g. "REFERENCES" or "STATEMENT STATUS FIELDS". */
  label: string;
  fields: TraceEntry[];
}
...
interface TracePanelModel {
  /** Optional — review items occasionally lack a trace (3 of 234 in baseline). */
  source?: TraceSource;
  identity?: TraceIdentity;
  linkedRecord?: TraceLinkedRecord;
  /** Multiple detail groups allowed (e.g. References + Statement status fields). */
  details?: TraceDetailGroup[];
  notes?: TraceNote[];

  // ---- Phase 3 reserved (intentionally unused in v2.2) ------
...
function TracePanel({
  ...
  onFocusTransaction?: (txId: string) => void;
}) {
  // Top section — source + identity merged into one grid for density.
  // Source is optional (review items occasionally lack a trace).
  const topFields: TraceEntry[] = [
    ...(model.source ? [
      { label: 'Source File',  value: model.source.file,  mono: true },
      ...(model.source.sheet ? [{ label: 'Source Sheet', value: model.source.sheet, mono: true }] : []),
      { label: 'Source Row',   value: String(model.source.row), mono: true },
    ] : []),
  ...
  return (
    <div className="trace-panel">
      <div className="trace-grid">
        {topFields.map((e, i) => (
          <TraceField key={`top-${e.label}-${i}`}
            label={e.label} value={e.value}
            mono={e.mono} small={e.small} />
        ))}
      </div>

      {/* Linked record (cross-tab focus target). */}
      {model.linkedRecord && (
        <div className="trace-refs">
          <div className="trace-section-label">{model.linkedRecord.label}</div>
          <div className="trace-grid">
            {model.linkedRecord.fields.map((e, i) => (
              <TraceField key={`linked-${e.label}-${i}`}
                label={e.label} value={e.value}
                mono={e.mono} small={e.small} />
            ))}
          </div>
          {onFocusTransaction && (
            <button
              ...
              onClick={() => onFocusTransaction(model.linkedRecord!.jumpTxId)}
            >
              {model.linkedRecord.jumpLabel}
            </button>
          )}
        </div>
      )}

      {/* Detail groups (references + raw status fields, etc.). */}
      {model.details?.map((group, gi) => (
        group.fields.length > 0 ? (
          <div className="trace-refs" key={`group-${group.label}-${gi}`}>
            <div className="trace-section-label">{group.label}</div>
            <div className="trace-grid">
              {group.fields.map((e, fi) => (
                <TraceField key={`${group.label}-${e.label}-${fi}`}
                  label={e.label} value={e.value}
                  mono={e.mono} small={e.small} />
              ))}
            </div>
      ...
      {/* Long-form notes (e.g. review reason text). */}
      {model.notes?.map((note, ni) => (
        <div className="trace-refs" key={`note-${note.label}-${ni}`}>
          <div className="trace-section-label">{note.label}</div>
          <p className="review-reason-detail">{note.text}</p>
        </div>
      ))}

      {/* ---- Phase 3 reserved render slots (deliberately not implemented) ----
       * model.activity   -> Activity log section (emails, payments, status changes)
       * model.status     -> Current Collection Status pill
       * model.nextAction -> Recommended next step
       *
       * Adding these in Phase 3 is a pure-additive change — no v2.2
       * call site needs to update.
       */}
    </div>
  );
}
...
function StatementsTab({ detail, onFocusTransaction }: StatementsTabProps) {
  ...
  type StatementLink = PartyDetail['statementLinks'][number];

  const isStrictNotInErp = useCallback(
    (link: StatementLink) =>
  ...
  }, [detail.statementLinks, source, match, currency, query, isStrictNotInErp]);
    ...
    <div className="party-tab-content">
      ...
      <div className="party-tab-footer stmt-footer">
        ...
        <button
          type="button"
          className="btn party-tab-export-btn"
          onClick={() => exportPartyTabCsv(detail, 'statements')}
          aria-label="Export statement rows to CSV"
        >
          Export CSV ↓
        </button>
      </div>
...
function StatementTracePanel({
  ...
  const refFields: TraceEntry[] = [];
  if (link.ourRefNo)  refFields.push({ label: 'ourRefNo',  value: link.ourRefNo,  mono: true });
  if (link.invoiceNo) refFields.push({ label: 'invoiceNo', value: link.invoiceNo, mono: true });
  if (link.crdrNo)    refFields.push({ label: 'crdrNo',    value: link.crdrNo,    mono: true });
  ...
  const detailGroups: TraceDetailGroup[] = [];
  if (refFields.length > 0) {
    detailGroups.push({ label: 'REFERENCES', fields: refFields });
  }
  const statusFields: TraceEntry[] = [];
  if (link.referenceStatus) statusFields.push({ label: 'referenceStatus', value: link.referenceStatus, mono: true });
  if (link.differenceType)  statusFields.push({ label: 'differenceType',  value: link.differenceType,  mono: true });
  if (statusFields.length > 0) {
    detailGroups.push({ label: 'STATEMENT STATUS FIELDS', fields: statusFields });
  }

  const linkedRecord: TraceLinkedRecord | undefined = matchedTx ? {
    label: 'ERP MATCH',
    fields: [
      { label: 'Tx Source File', value: matchedTx.trace.sourceFile, mono: true },
      { label: 'Tx Source Row',  value: String(matchedTx.trace.sourceRow), mono: true },
      { label: 'Tx Signed',      value: formatSignedMoney(matchedTx.signedBalance), mono: true },
    ...
    jumpTxId: matchedTx.id,
    jumpLabel: 'View this transaction in Transactions tab →',
  } : undefined;
  ...
  const model: TracePanelModel = {
    ...
    identity: {
      fields: [
        ...
        { label: 'Party Name',   value: partyName },
        { label: 'Party Key',    value: link.partyKey, mono: true },
        { label: 'Match Type',   value: matchTypeLabel },
        { label: 'Currency',     value: link.currency, mono: true },
        { label: 'Stmt Balance', value: formatSignedMoney(link.statementBalance), mono: true },
      ],
    },
    linkedRecord,
  ...
  return <TracePanel model={model} onFocusTransaction={onFocus} />;
}
    ...
    <div className="party-tab-content">
      ...
      <div className="party-tab-footer">
        ...
        <button
          type="button"
          className="btn party-tab-export-btn"
          onClick={() => exportPartyTabCsv(detail, 'reviews')}
          aria-label="Export review items to CSV"
        >
          Export CSV ↓
        </button>
      </div>
...
function ReviewTracePanel({
  ...
  const extraEntries: TraceEntry[] = [];
  if (item.details) {
    for (const [k, v] of Object.entries(item.details)) {
      if (v == null) continue;
      if (typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean') {
        extraEntries.push({ label: k, value: String(v), mono: true });
      }
    }
  }

  const identityFields: TraceEntry[] = [
    { label: 'Category',    value: CATEGORY_LABELS[item.category as keyof typeof CATEGORY_LABELS] ?? item.category },
    { label: 'Reason Code', value: item.reasonCode, mono: true },
    { label: 'Severity',    value: item.severity },
    { label: 'Party Name',  value: partyName },
    { label: 'Party Key',   value: item.partyKey ?? '—', mono: true },
    ...(item.currency ? [{ label: 'Currency', value: item.currency, mono: true }] : []),
    ...(item.amount !== null ? [{ label: 'Amount', value: formatSignedMoney(item.amount), mono: true }] : []),
  ];

  const detailGroups: TraceDetailGroup[] = [];
  if (extraEntries.length > 0) {
    detailGroups.push({ label: 'DETAILS', fields: extraEntries });
  }
  ...
  const linkedRecord: TraceLinkedRecord | undefined = matchedTx ? {
    ...
    fields: [
      { label: 'Tx Source File', value: matchedTx.trace.sourceFile, mono: true },
      { label: 'Tx Source Row',  value: String(matchedTx.trace.sourceRow), mono: true },
      { label: 'Tx Type',        value: matchedTx.sourceType },
      { label: 'Tx Direction',   value: matchedTx.direction },
      { label: 'Tx Signed',      value: formatSignedMoney(matchedTx.signedBalance), mono: true },
    ],
    jumpTxId: matchedTx.id,
    jumpLabel: 'View this transaction in Transactions tab →',
  ...
  const model: TracePanelModel = {
    source: item.trace ? {
      file: item.trace.sourceFile,
      sheet: item.trace.sourceSheet,
      row: item.trace.sourceRow,
    } : undefined,
    identity: { fields: identityFields },
    linkedRecord,
    details: detailGroups,
    notes: [{ label: 'REASON', text: item.reason }],
  };
    ...
    <div className="party-tab-content">
      ...
      <div className="party-tab-footer">
        ...
        <button
          type="button"
          className="btn party-tab-export-btn"
          onClick={() => exportPartyTabCsv(detail, 'duplicates')}
          aria-label="Export duplicate groups to CSV"
        >
          Export CSV ↓
        </button>
      </div>
