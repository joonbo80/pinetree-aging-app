# v2.3 C2 Round 1 Fix Package -- Selector + Data Invariants (rev2)
...
- rev2 (2026-05-12): Reviewer P1 fixes:
  - P1-1: Tautological invariant fix. The rev1 test re-implemented
    ...
    asserts on real selector output (`report.tabs.current.transactionCount`,
    `report.noDueDate.openCount`, etc.).
  - P1-2: AR/AP netting prevention. The rev1 selector grouped party
    rollups by (partyKey, currency), which would net AR (receivable)
    against AP (payable) when the same party had both open balances
    under the same currency. rev2 adds `direction` to the grouping
    key, so AR and AP appear as SEPARATE rollup rows. Verified
    against 6 real baseline parties with mixed AR+AP balances.
  - New PartyRollup.direction and TransactionRow.direction fields.
  - 36 sub-checks total (was 24 in rev1); broken state catches 13.
...
## Swap-test proof of invariant authenticity (rev2)

```
BROKEN state (selector source file absent): PASS:  6    FAIL: 13
FIXED  state (selector source file present): PASS: 36   FAIL:  0
```

The 6 PASSes on broken state are baseline-fact invariants that grep
...
If the fixture is regenerated with different data, those invariants
fail even with a working selector.

The 13 FAILs on broken state are:
- 12 source-level invariants that require the selector .ts file
  to exist and have the correct structure.
- 1 import invariant that requires the selector module to load.

When the selector loads (FIXED state), the test additionally runs
~17 runtime invariants that assert on REAL selector output (tab
counts, AR/AP separation, Cleared band guard, threshold values).
This is the rev2 fix for P1-1: the test now exercises the selector
in-flight, not its own re-implementation of the rules.

Together, the two layers protect against:

- Selector drift (source + import + runtime invariants FAIL if the
  selector source is removed, mutated incorrectly, or outputs wrong
  values).
- Baseline drift (baseline-fact invariants FAIL if the v1.3.0
  fixture is regenerated with different statementLink counts).
...
- [ ] `node --experimental-strip-types .\tools\test-aging-report-view.mjs` returns PASS: 36 / FAIL: 0.
